    ...
    def test_process_translation_with_page_range(self):
        ...
        try:
            ...
            with patch('services.translation_service.PdfExtractor') as mock_pdf_extractor_class:
                ...
                # extract() 现在返回 (PdfExtraction, missing_pages) 元组
                mock_pdf_extractor.extract.return_value = (extracted_content, [])
                mock_pdf_extractor.total_pages = 3
                mock_pdf_extractor_class.return_value = mock_pdf_extractor

                # 模拟翻译器
                with patch.object(translation_service, 'get_translator') as mock_get_translator, \
                     patch.object(translation_service, 'get_semantic_analyzer') as mock_get_analyzer:
                    mock_translator = MagicMock()
                    # translate 返回带有 content 和 truncated 属性的对象
                    mock_translation_result = MagicMock()
                    mock_translation_result.content = "Translated text"
                    mock_translation_result.truncated = False
                    mock_translator.translate.return_value = mock_translation_result
                            ...
                            with patch('services.translation_service.PdfGenerator') as mock_pdf_generator:
                                ...
                                with patch('services.translation_service.remove_file') as mock_remove_file:
                                    ...
                                    translation_service.process_translation(
                                        ...
                                        "test_unique_id", "test_file.pdf",
                                        doc_type="技术文档", glossary="", page_range=""
                                    )
                                    ...
                                    translation_service.process_translation(
                                        mock_task, temp_pdf_path, "en", "zh", "silicon_flow",
                                        "test_unique_id", "test_file.pdf",
                                        doc_type="技术文档", glossary="", page_range="1"
                                    )
                                    ...
                                    translation_service.process_translation(
                                        mock_task, temp_pdf_path, "en", "zh", "silicon_flow",
                                        "test_unique_id", "test_file.pdf",
                                        doc_type="技术文档", glossary="", page_range="1-2"
                                    )
    ...
    def test_process_translation_with_no_matching_pages(self):
        ...
        try:
            ...
            with patch('services.translation_service.PdfExtractor') as mock_pdf_extractor_class:
                ...
                # extract() 现在返回 (PdfExtraction, missing_pages) 元组
                mock_pdf_extractor.extract.return_value = (extracted_content, [])
                mock_pdf_extractor.total_pages = 2
                mock_pdf_extractor_class.return_value = mock_pdf_extractor
                ...
                translation_service.process_translation(
                    mock_task, temp_pdf_path, "en", "zh", "silicon_flow",
                    "test_unique_id", "test_file.pdf",
                    doc_type="技术文档", glossary="", page_range="3"
                )

...
- 翻译提示词优化（3项）：
  - **规则10扩展为"代码段保留与格式化"**：增加代码格式化要求（保持缩进、空格、换行、对齐），增加代码特征识别说明（编程关键字和特殊符号），增加代码注释不翻译要求
  - **规则10特征描述修复**：移除"连续的多行缩进文本"和"特殊缩进结构的文本"描述（提取阶段已删除换行符，输入中不存在多行结构），仅保留编程关键字和特殊符号特征
  - **规则14强化为"禁止元注释和原文输出"**：增加"严禁输出原文内容"和"不保留原文"，明确禁止翻译结果输出英文原文
- 换行符清理策略微调：将提取阶段的 `\n` 替换从空字符串改为空格（`text.replace('\n', '')` → `text.replace('\n', ' ')`），避免换行符删除后相邻单词直接粘连
- 更新测试断言以匹配新的规则文本
- 相关文件：`modules/translator.py`、`modules/ocr/paddle_extractor.py`、`modules/pdf_extractor.py`、`modules/extractors/coordinate_utils.py`、`tests/test_newline_preservation.py`
